...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.video:
        # Keep exported evidence centered on the moving robot. This changes
        # only the synthetic camera frame; policy observations, dynamics and
        # the robot's world trajectory remain untouched.
        env_cfg.viewer.origin_type = "asset_root"
        env_cfg.viewer.asset_name = "robot"
        env_cfg.viewer.env_index = max(0, args_cli.play_env_id)
