...
def main() -> None:
    ...
    if xy_drift is not None:
        ...
        diagnostic_windows = {
            "initial_5pct": tracking_window_stats(time_s, xy_drift, 0.0, 0.05 * duration_s),
            "middle_5pct": tracking_window_stats(
                time_s, xy_drift, 0.475 * duration_s, 0.525 * duration_s
            ),
            "final_10pct": tracking_window_stats(time_s, xy_drift, 0.90 * duration_s, duration_s),
        }
        # These windows describe dance1-specific motion phases and must not be
        # applied to longer or unrelated trajectories such as dance2.
        if motion_total_frames == 3972:
            diagnostic_windows.update({
    ...
    summary = {
        ...
        "platform_position": {
            ...
            "max_abs_axis_position_m": float(np.max(np.abs(actual_xy))),
            "minimum_edge_margin_m": float(
                args.platform_size * 0.5 - np.max(np.abs(actual_xy))
            ),
            ...
            "reference_start_xy_error_m": float(xy_drift[0]) if xy_drift is not None else None,
        ...
        "quality_checks": {
            ...
            "started_near_reference": bool(xy_drift is None or xy_drift[0] < 0.05),
            "within_platform_bounds": bool(
                np.max(np.abs(actual_xy)) < args.platform_size * 0.5
            ),
        },
    }
