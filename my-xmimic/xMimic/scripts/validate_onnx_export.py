...
def checkpoint_actor_forward(
    ...
    normalizer_state: dict[str, torch.Tensor] | None,
) -> np.ndarray:
...
def checkpoint_actor_normalizer(
    checkpoint: dict, expected_obs: int
) -> tuple[str, dict[str, torch.Tensor] | None]:
    """Resolve all checkpoint layouts supported by the local RSL-RL versions."""
    if "obs_norm_state_dict" in checkpoint:
        state = checkpoint["obs_norm_state_dict"]
        mode = "legacy_top_level"
    else:
        ...
        mode = "embedded_actor_critic" if state else "identity"
        if not state:
            return mode, None
    required = {"_mean", "_var", "_std", "count"}
    if not required <= state.keys() or list(state["_mean"].shape) != [1, expected_obs]:
        raise ValueError(f"Unsupported {mode} actor normalizer layout")
    return mode, state
...
def main() -> None:
    ...
    normalizer_mode, obs_normalizer = checkpoint_actor_normalizer(
        checkpoint, args.expected_obs
    )
    if obs_normalizer is None:
        expected_mean = np.zeros((1, args.expected_obs), dtype=np.float32)
        expected_std = np.ones((1, args.expected_obs), dtype=np.float32)
        expected_divisor = None
        normalizer_mean_matches = not any(
            name.startswith("normalizer.") for name in initializer_arrays
        )
    else:
        expected_mean = obs_normalizer["_mean"].detach().cpu().numpy()
        expected_std = obs_normalizer["_std"].detach().cpu().numpy()
        expected_divisor = expected_std + 1.0e-2
    ...
    divisor_matches = [
        ...
        and expected_divisor is not None
    ...
    observation = expected_mean + expected_std * rng.standard_normal(
    ...
    checks = {
        ...
        "normalizer_divisor_matches_checkpoint": (
            len(divisor_matches) == 0
            if normalizer_mode == "identity"
            else len(divisor_matches) == 1
        ),
    ...
    document = {
        ...
        "actor_normalizer_mode": normalizer_mode,
