...
def flatten_record(label: str, document: dict) -> dict:
    ...
    return {
        ...
        "started_near_reference": checks.get("started_near_reference"),
        "within_platform_bounds": checks.get("within_platform_bounds"),
        ...
        "max_abs_axis_position_m": position.get("max_abs_axis_position_m"),
        "minimum_edge_margin_m": position.get("minimum_edge_margin_m"),
...
def acceptance_failures(record: dict) -> list[str]:
    ...
    def optional_at_most(value: float | None, limit: float) -> bool:
    ...
    checks = [
        ...
        ("started_near_reference", record["started_near_reference"] is True),
        ("within_platform_bounds", record["within_platform_bounds"] is True),
        (
            "platform_edge_margin",
            record["minimum_edge_margin_m"] is not None
            and record["minimum_edge_margin_m"] >= 0.25,
        ),
        ...
        (
            ...
            optional_at_most(record["initial_stationary_xy_max_m"], initial_stationary_limit),
        ),
        (
            "stationary_tail_xy_increase",
            optional_at_most(record["stationary_tail_xy_increase_m"], 0.05),
        ),
        (
            "stationary_tail_xy_slope",
            optional_at_most(record["stationary_tail_xy_slope_m_per_s"], 0.01),
        ),
    ]
...
def main() -> None:
    ...
    comparison = {
        "criteria": {
            ...
            "shared": {
                "platform_edge_margin_min_m": 0.25,
    ...
    bar_chart(
        axes[1, 2],
        labels,
        [float(record["minimum_edge_margin_m"] or 0.0) for record in records],
        "Minimum platform-edge margin",
        "Margin (m)",
        colors=result_colors,
        limits=[shared_limits["platform_edge_margin_min_m"]] * len(records),
    )
    figure.legend(
        handles=[
