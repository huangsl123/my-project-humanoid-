...
class RobotSimulator:
    ...
    def __init__(self, model_path, node: Node | None = None, debug=False, robot_config='full'):
        ...
        if self.node:
            ...
            self.imu_status_pub = self.node.create_publisher(
                Imu, 'imu/status', 10)
            # Publish the floating-base state that is already available from
            # MuJoCo.  BeyondMimic policies trained with state estimation need
            # the pelvis world position and linear velocity; the Imu message
            # has no fields for those values.
            self.base_odometry_pub = self.node.create_publisher(
                Odometry, 'base/odometry', 10)
        # 传感器返回的机器人状态
    ...
    def _publish_thread(self):
        ...
        while self.pub_thread_running and not self.stop_event.is_set():
            ...
            self._publish_base_odometry(self.base_odometry_pub, sensor_data)
            # 控制发布频率
    ...
    def _publish_base_odometry(self, publisher, sensor_data):
        """Publish pelvis pose and world-frame linear velocity."""
        if not publisher or not sensor_data:
            return

        orientation = sensor_data['imu_orientation']
        position = sensor_data['imu_position']
        linear_velocity = sensor_data['imu_linear_velocity']
        msg = Odometry()
        ...
        try:
            ...
            print(f"发布 base odometry 时出错: {e}")
