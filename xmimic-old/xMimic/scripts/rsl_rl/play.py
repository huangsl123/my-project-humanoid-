...
def _disable_manager_terms(cfg: object) -> None:
    """Disable all configured terms for deterministic policy evaluation."""
    if cfg is None:
        return
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # Evaluation and deployment export use nominal dynamics and observations.
    env_cfg.observations.policy.enable_corruption = False
    _disable_manager_terms(env_cfg.events)

    # Keep evaluation and recorded video clean and centered on the tracked
    # robot. Motion/contact debug markers otherwise flash on every body, and a
    # world-fixed camera can lose motions that translate across the terrain.
    env_cfg.commands.motion.debug_vis = False
    env_cfg.scene.contact_forces.debug_vis = False
    env_cfg.viewer.eye = (1.8, 1.8, 0.8)
    env_cfg.viewer.lookat = (0.0, 0.0, 0.0)
    env_cfg.viewer.origin_type = "asset_body"
    env_cfg.viewer.env_index = args_cli.play_env_id
    env_cfg.viewer.asset_name = "robot"
    env_cfg.viewer.body_name = "pelvis"
    ...
    # Isaac Lab 5.0 may return (observations, extras).
