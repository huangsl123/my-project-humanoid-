...
class FSMStateBeyondMimic(FSMState):
    def __init__(self, robot_data: RobotData, config_path: Optional[str] = None, variant_name: str = "default"):
        ...
        with open(self.config_path, "r") as f:
            ...
            # State-feedback policies add motion_anchor_pos_b and base_lin_vel
            # (3 values each).  ``measured`` consumes xSIM /base/odometry;
            # ``reference`` remains only as a legacy best-effort fallback.
            self.state_feedback_source = config.get("state_feedback_source", "disabled")
            if self.state_feedback_source not in ("disabled", "reference", "measured"):
                raise ValueError(
                    "state_feedback_source must be 'disabled', 'reference', or 'measured'"
                )
    ...
    def on_enter(self):
        ...
        self._final_ref_cached = False
        self._ref_to_world_translation = None
    ...
    def inner_run(self):
        ...
        if(self.counter_step < 2):
            ...
            if self.state_feedback_source == "measured":
                if not self.robot_data_.base_state_received_:
                    raise RuntimeError(
                        "state_feedback_source=measured but /base/odometry has not been received"
                    )
                self._ref_to_world_translation = (
                    self.robot_data_.get_base_position_w()
                    - self.init_to_world @ ref_anchor_pos_w
                )
        ...
        state_feedback_vec = np.empty(0, dtype=np.float32)
        if self.state_feedback_source == "reference":
            # Legacy best-effort mode approximates perfect tracking for these
            # two states.  Keep the exact training order:
            # motion_anchor_pos_b, then base_lin_vel.
            motion_anchor_pos_b = np.zeros(3, dtype=np.float32)
            ref_anchor_lin_vel_w = self.ref_body_lin_vel_w[0, self.anchor_body_index]
            base_lin_vel = (
                robot_rot_mat.T @ self.init_to_world @ ref_anchor_lin_vel_w
            ).astype(np.float32)
            state_feedback_vec = np.concatenate(
                (motion_anchor_pos_b, base_lin_vel), dtype=np.float32
            )
        elif self.state_feedback_source == "measured":
            if not self.robot_data_.base_state_received_:
                raise RuntimeError("Lost required /base/odometry state")
            if self._ref_to_world_translation is None:
                raise RuntimeError("Reference-to-world translation was not initialized")
            base_position_w = self.robot_data_.get_base_position_w()
            base_linear_velocity_w = self.robot_data_.get_base_linear_velocity_w()
            aligned_ref_anchor_pos_w = (
                self._ref_to_world_translation
                + self.init_to_world @ ref_anchor_pos_w
            )
            # Same convention as Isaac Lab's subtract_frame_transforms:
            # target anchor position relative to the actual pelvis, expressed
            # in the actual pelvis frame.
            motion_anchor_pos_b = (
                robot_rot_mat.T @ (aligned_ref_anchor_pos_w - base_position_w)
            ).astype(np.float32)
            base_lin_vel = (
                robot_rot_mat.T @ base_linear_velocity_w
            ).astype(np.float32)
            state_feedback_vec = np.concatenate(
                (motion_anchor_pos_b, base_lin_vel), dtype=np.float32
            )
        ...
        # Retain one diagnostic copy so deployment validators can prove field
        # ordering and coordinate semantics without changing policy inference.
        self.last_policy_observation = np.array(mimic_obs_buf, copy=True)
