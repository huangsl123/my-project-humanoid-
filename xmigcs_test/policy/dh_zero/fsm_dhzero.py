...
class FSMStateDHZero(FSMState):
    """零位状态实现（完整C++逻辑移植）"""
    def __init__(self, robot_data: RobotData):
        super().__init__(robot_data)
        self.current_state_name = FSMStateName.DHZERO
        self.q_factor_ = 0.0
        # 获取包路径
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "config", "dh_zero.yaml")
        ...
        try:
            ...
            print(f"[FSMStateDHZERO] YAML load error: {e}")
            self.action_num_ = 12
            self.motor_num_ = 29
    ...
    def on_enter(self):
        print("[FSMStateDHZERO] Enter DHZERO state")
        self.q_factor_ = 0.0
    ...
    def on_exit(self):
        print("[FSMStateDHZERO] Exit dhzero position control state")
