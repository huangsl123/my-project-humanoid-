...
from xmigcs.policy.dh.base_agent import baseAgent
from xmigcs.policy.dh.model import MODEL_DIR
import os.path as osp
import numpy as np
from xmigcs.policy.dh.exp_filter import expFilter
import onnxruntime as ort


class dex_stand_Agent(baseAgent):
    def __init__(self):
        self.num_prop_obs_input = 93
        self.include_history_steps = 10
        self.num_actions = 29
        policy_path = osp.join(MODEL_DIR, "dex_1030_stand.onnx")
        ...
        self.prop_obs_history = np.zeros((self.num_prop_obs_input, self.include_history_steps))
    ...
    def inference(self, obs_group):
        ...
        input_feed = {
            "input": prop_obs_history.flatten()[None, :].astype(np.float32),
        }
    ...
    def reset(self):
        ...
        self.inference_count = 0
        norminal_obs = self.build_observations_one_step(norminal_obs_group)
