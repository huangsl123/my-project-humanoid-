...
from common.func_sp_trans_new import OptimizedFuncSPTrans as OptimizedFuncSPTransNew
...
def test_parallel_to_serial_performance():
    ...
    speedup = dex_avg_time / new_avg_time if new_avg_time > 0 else float('inf')
    
    print(f"性能统计 (运行 {num_runs} 次):")
    print(f"  DEX优化版本总时间: {dex_total_time:.4f} 秒")
    print(f"  NEW优化版本总时间: {new_total_time:.4f} 秒")
    print(f"  DEX优化版本平均时间: {dex_avg_time*1000:.6f} ms")
    print(f"  NEW优化版本平均时间: {new_avg_time*1000:.6f} ms")
    print(f"  性能提升: {speedup:.2f}x")
    print(f"  DEX优化版本平均频率: {1.0/dex_avg_time:.2f} Hz")
