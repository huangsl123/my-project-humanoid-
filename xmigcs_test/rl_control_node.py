...
class XMIGCSControlNode(Node):
    ...
    def _rl_control_loop(self):
        ...
        while self.control_running and rclpy.ok():
            ...
            self.robot_fsm.run_fsm(self.control_flag)

            # 发布控制命令
            self._send_control_commands(self.control_flag)
    ...
    def _wait_for_start_signal(self):
        """等待启动信号"""
        start_file = os.environ.get("XMIGCS_START_SIGNAL", "/tmp/rl_start_signal")
        self.get_logger().info("Waiting for start signal...")
        self.get_logger().info(f"Run: touch {start_file}")
