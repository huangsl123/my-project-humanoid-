...
import numpy as np
# ROS messages
from bodyctrl_msgs.msg import MotorStatusMsg, CmdMotorCtrl, MotorCtrl, Imu
...
from common.func_sp_trans_evt_opt import OptimizedFuncSPTrans as FuncSPTrans
...
class RobotInterfaceImpl(RobotInterface):
    ...
    def __init__(self, robot_data: RobotData, config_path: str = ''):
        ...
        # 消息队列
        self.queue_leg_motor_state = queue.Queue(maxsize=1)
        self.queue_arm_motor_state = queue.Queue(maxsize=1)
        self.queue_waist_motor_state = queue.Queue(maxsize=1)
        self.queue_imu_xsens = queue.Queue(maxsize=1)
        self.queue_base_odometry = queue.Queue(maxsize=1)
        self.queue_walk_cmd = queue.Queue(maxsize=1)
    ...
    def load_config(self):
        ...
        if not os.path.exists(config_path):
            self.node.get_logger().error(f"Joint limits config file not found: {config_path}")
            raise FileNotFoundError(f"Joint limits config file not found: {config_path}")
            
        try:
            with open(config_path, 'r') as f:
                config = yaml.safe_load(f)
        except Exception as e:
            self.node.get_logger().error(f"Failed to load joint limits config from {config_path}: {e}")
            raise RuntimeError(f"Failed to load joint limits config from {config_path}: {e}")
        ...
        # IMU 数据偏移
        self.xsense_roll_offset = interface_config.get('xsense_data_roll_offset')
        # 禁用电机
    ...
    def _load_control_status(self, config: Dict[str, Any]):
        self.waist_control_status = config.get('waist_control_status')
        self.legs_control_status = config.get('legs_control_status')
        self.arms_control_status = config.get('arms_control_status')
    ...
    def _load_joint_limits(self, config: Dict[str, Any]):
        ...
        # 记录加载的限位值
        for joint_name, limits in self.joint_limits.items():
            self.node.get_logger().debug(f"Joint {joint_name}: [{limits['min']}, {limits['max']}]")
        print("-"*30 + '关节限位值' + '-'*30)
    ...
    def _init_ros_interfaces(self):
        ...
        self.sub_leg_state = self.node.create_subscription(
            MotorStatusMsg, '/leg/status', self._leg_motor_status_callback, qos_profile)
        self.sub_arm_state = self.node.create_subscription(
            MotorStatusMsg, '/arm/status', self._arm_motor_status_callback, qos_profile)
        self.sub_waist_state = self.node.create_subscription(
            MotorStatusMsg, '/waist/status', self._waist_motor_status_callback, qos_profile)
        
        #（非电机相关）
        self.sub_imu_xsens = self.node.create_subscription(
            Imu, '/imu/status', self._imu_status_callback, qos_profile)
        self.sub_base_odometry = self.node.create_subscription(
            Odometry, '/base/odometry', self._base_odometry_callback, qos_profile)
        
        # roy 添加你的订阅话题
        self.sub_walk_cmd = self.node.create_subscription(
            TwistStamped, '/hric/robot/cmd_vel', self._walk_cmd_callback, qos_profile)
    ...
    def get_imu_data(self):
        ...
        while True:
            try:
                msg = self.queue_imu_xsens.get_nowait()
    ...
    def get_base_odometry(self):
        """Consume the newest MuJoCo floating-base state without blocking."""
        newest = None
        while True:
            try:
                newest = self.queue_base_odometry.get_nowait()
            except queue.Empty:
                break
        if newest is None:
            return
        self.robot_data_.base_position_w_[:] = (
            newest.pose.pose.position.x,
            newest.pose.pose.position.y,
            newest.pose.pose.position.z,
        )
        self.robot_data_.base_linear_velocity_w_[:] = (
            newest.twist.twist.linear.x,
            newest.twist.twist.linear.y,
            newest.twist.twist.linear.z,
        )
        self.robot_data_.base_state_received_ = True
    ...
    def update_robot_state(self):
        ...
        while True:
            try:
                msg = self.queue_leg_motor_state.get_nowait()
                if self.debug:
                ...
                for status in msg.status:
                    self.motor_state_to_robot_state(status, self.first_leg_data_received)
                break
            except queue.Empty:
                time.sleep(0.0001)
        ...
        while True:
            try:
                msg = self.queue_arm_motor_state.get_nowait()
                if self.debug:
                    current_time = self.node.get_clock().now().to_msg()
                    msg_time = msg.header.stamp
                    time_diff = (current_time.sec - msg_time.sec) * 1000000000 + (current_time.nanosec - msg_time.nanosec)
                    ...
                    print(f"Time difference: {time_diff_ms} ms")
                for status in msg.status:
                    self.motor_state_to_robot_state(status, self.first_arm_data_received)
        ...
        while True:
            try:
                msg = self.queue_waist_motor_state.get_nowait()
                if self.debug:
                    current_time = self.node.get_clock().now().to_msg()
                    msg_time = msg.header.stamp
                    time_diff = (current_time.sec - msg_time.sec) * 1000000000 + (current_time.nanosec - msg_time.nanosec)
                    ...
                    print(f"Time difference: {time_diff_ms} ms")
                for status in msg.status:
                    self.motor_state_to_robot_state(status, self.first_waist_data_received)
                break
            except queue.Empty:
                time.sleep(0.0001)
    def get_walk_cmd(self):
        ...
        try:
            ...
            self.robot_data_.walk_cmd_[0] = msg.twist.linear.x
            self.robot_data_.walk_cmd_[1] = msg.twist.linear.y
            self.robot_data_.walk_cmd_[2] = msg.twist.angular.z
        except queue.Empty:
    ...
    def motor_state_to_robot_state(self, status, received_flag: bool):
        ...
        if index >= 0:
            ...
            self.robot_data_.tau_a_[robotdata_index] = status.current * self.ct_scale[min(index, len(self.ct_scale)-1)]
            self.robot_data_.temperature_a[robotdata_index - self.floating_base_dof] = status.temperature

            self.robot_data_.q_a_[robotdata_index] = (status.pos - self.zero_pos[index]) * self.motor_dir[index] + self.zero_offset[index]
            # MuJoCo reports the actual bounded hinge-joint angle directly.
            # Applying the hardware encoder wrap correction in simulation can
            # turn a valid angle just below -pi into a target shifted by -2pi
            # (for example, -3.04 -> -7.37 rad on r_shoulder_roll), producing
            # a one-cycle command jump and actuator saturation.  Keep encoder
            # unwrapping only for the real robot interface.
            if self.sim:
                self.zero_cnt[index] = 0.0
            elif self.robot_data_.q_a_[robotdata_index] > math.pi:
                self.zero_cnt[index] = -1.0
            elif self.robot_data_.q_a_[robotdata_index] < -math.pi:
                self.zero_cnt[index] = 1.0

            self.robot_data_.q_a_[robotdata_index] += self.zero_cnt[index] * 2.0 * math.pi
            ...
            if not received_flag or abs(self.robot_data_.q_a_[robotdata_index] - self.robot_data_.q_a_last[robotdata_index]) > self.joint_pos_threshold:
                if received_flag:
                    self.node.get_logger().warn(f"Joint {index} error detected")
                    self.robot_data_.q_a_[robotdata_index] = self.robot_data_.q_a_last[robotdata_index]
                    self.robot_data_.q_dot_a_[robotdata_index] = self.robot_data_.qdot_a_last[robotdata_index]
                    self.robot_data_.tau_a_[robotdata_index] = self.robot_data_.tor_a_last[robotdata_index]
                else:
                    # 首次接收数据，更新标志位
                    received_flag = True
            self.robot_data_.q_a_last[robotdata_index] = self.robot_data_.q_a_[robotdata_index]
            self.robot_data_.qdot_a_last[robotdata_index] = self.robot_data_.q_dot_a_[robotdata_index]
            self.robot_data_.tor_a_last[robotdata_index] = self.robot_data_.tau_a_[robotdata_index]

    def update_sensor_states(self):
        # 获取Imu数据
        self.get_imu_data()
        self.get_base_odometry()
        # 添加IMU偏移
    ...
    def _check_and_clip_joint_limits_fast(self, cmd_name: int, position: float) -> tuple[bool, float]:
        """
    ...
    def convert_to_motor_commands(self):
        ...
        np.add(self.temp_q, self.zero_pos, out=self.robot_data_.q_d_[self.floating_base_dof:])
        
        # qdot_d和tor_d的计算也可以向量化
        np.multiply(qdot_d_reordered, self.motor_dir, out=self.robot_data_.q_dot_d_[self.floating_base_dof:])
        np.multiply(tor_d_reordered, self.motor_dir, out=self.robot_data_.tau_d_[self.floating_base_dof:])
    ...
    def publish_motor_commands(self, flag: ControlFlag):
        ...
        # 发布腿部控制命令
        if self.legs_control_status == [] or flag_fsm_command in self.legs_control_status:
            ...
            for i in range(self.id_map.leg_motor_nums):
                ...
                # 检查关节位置限位
                within_limit, cmd.pos = self._check_and_clip_joint_limits_fast(cmd.name, cmd.pos)
                if not within_limit:
                    print(f"Joint (id: {cmd.name}) position {cmd.pos} is out of limits")
                leg_msg.cmds.append(cmd)
            self.pub_leg_motor_cmd.publish(leg_msg)

        # 只在特定模式下控制腰部
        if flag_fsm_command in self.waist_control_status:
            # 腰部控制命令
            ...
            for i in range(self.id_map.waist_motor_nums):
                ...
                cmd.name = self.id_map.get_id_by_index(motor_idx) # 12 -> 33(yaw)
        ...
        # 只在特定模式下控制手臂
        if flag_fsm_command in self.arms_control_status:
            # 手臂控制命令
            arm_msg = CmdMotorCtrl()
            arm_msg.header.stamp = self.node.get_clock().now().to_msg()
            for i in range(self.id_map.arm_motor_nums):
                cmd = MotorCtrl()
    ...
    def publish_robot_cmd(self, flag: ControlFlag):
        """发布机器人控制命令"""
        # roy 在这里实现你的话题发布代码
        if flag.enable:
            # 发布行走控制命令
            walk_msg = TwistStamped()
            walk_msg.header.stamp = self.node.get_clock().now().to_msg()
    ...
    def _base_odometry_callback(self, msg):
        """Keep only the latest floating-base state."""
        try:
            self.queue_base_odometry.put_nowait(msg)
        except queue.Full:
            try:
                self.queue_base_odometry.get_nowait()
                self.queue_base_odometry.put_nowait(msg)
            except queue.Empty:
                pass
    ...
    def ankle_parallel_to_serial(self):
        ...
        q_a_p[:2] = self.robot_data_.q_a_[self.left_ankle_indices]  # 左脚踝关节 (pitch, roll)
        q_a_p[2:] = self.robot_data_.q_a_[self.right_ankle_indices]  # 右脚踝关节 (pitch, roll)
    ...
    def ankle_serial_to_parallel(self):
        ...
        q_d_s[2:] = self.robot_data_.q_d_[self.right_ankle_indices]  # 右脚踝关节串联命令
        ...
        qdot_a_s[:2] = self.robot_data_.q_dot_a_[self.left_ankle_indices] # 左脚踝关节串联速度
        qdot_a_s[2:] = self.robot_data_.q_dot_a_[self.right_ankle_indices] # 右脚踝关节串联速度
        ...
        q_d_p = (tor_d_p - self.ankle_kd_p * (qdot_d_p - self.qdot_a_p)) / self.ankle_kp_p + self.q_a_p
    ...
    def update_robot_cmd(self, flag: ControlFlag):
        ...
        if flag.enable:
            ...
            x_command = getattr(flag, 'x_speed_command', 0.0)
            y_command = getattr(flag, 'y_speed_command', 0.0)
            yaw_command = getattr(flag, 'yaw_speed_command', 0.0)
            self.robot_data_.walk_cmd_ = [x_command, y_command, yaw_command]

            if all(hasattr(flag, attr) for attr in ['waist_roll_command', 'waist_pitch_command', 'waist_yaw_command', 'height_cmd']):
                waist_roll_command = getattr(flag, 'waist_roll_command', 0.0)
                waist_pitch_command = getattr(flag, 'waist_pitch_command', 0.0)
                waist_yaw_command = getattr(flag, 'waist_yaw_command', 0.0)
                height_cmd = getattr(flag, 'height_cmd', 0.0)
                self.robot_data_.floating_base_cmd_ = [waist_roll_command, waist_pitch_command, waist_yaw_command, height_cmd]

        else:
            # roy 在这里把topic 收到的指令传递给robot_data中的命令变量
            self.get_walk_cmd()
