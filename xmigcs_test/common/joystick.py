...
class ControlFlag:
    ...
    # 禁用、启用手柄控制标志
    enable: bool = True
    beyondmimic_variant: str = "default"
...
class JoystickHumanoid:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.joy_flag.height_cmd = self.current_height
            
        except Exception as e:
            print(f"[Joystick_humanoid] YAML load error: {e}")
        ...
        with self.data_mutex:
            if len(msg.axes) >= 12:  # 确保有足够的轴数据
                yunzhuo_map = YUNZHUOMap(
                    ...
                    x2=msg.axes[1] if len(msg.axes) > 1 else 0.0,
                    y1=msg.axes[2] if len(msg.axes) > 2 else 0.0,
                    y2=msg.axes[0] if len(msg.axes) > 0 else 0.0)
    ...
    def joy_flag_update(self):
        ...
        with self.data_mutex:
            ...
            if self.joy_map.c == 1.0:
                ...
                button_pressed_nums = self.check_button_pressed_nums(self.joy_map)
                if button_pressed_nums == 0:
                    if self.joy_map.d == 1.0:
                        self.joy_flag.fsm_state_command = "gotoZERO"
                    elif self.joy_map.a == 1.0:
                        self.joy_flag.fsm_state_command = "gotoMLP"
                if button_pressed_nums == 1:
                    if self.joy_map.h == -1.0:
                        # h右拨
                        if self.joy_map.d == 1.0:
                            self.joy_flag.fsm_state_command = "gotoDHZERO"
                        elif self.joy_map.a == 1.0:
                            self.joy_flag.fsm_state_command = "gotoDH"
                    if self.joy_map.h == 1.0:
                        # h左拨
                        if self.joy_map.d == 1.0:
                            self.joy_flag.fsm_state_command = "gotoPBHCZERO"
                        elif self.joy_map.a == 1.0:
                            self.joy_flag.fsm_state_command = "gotoPBHC"

                    if self.joy_map.f == -1.0 and self.joy_map.a == 1.0:
                        # f上拨 + A -> down
                        self._set_beyondmimic_variant("down")
                    elif self.joy_map.f == 1.0 and self.joy_map.a == 1.0:
                        # f下拨 + A -> up
                        self._set_beyondmimic_variant("up")
                    elif self.joy_map.g == -1.0 and self.joy_map.a == 1.0:
                        # g上拨 + A -> dance
                        self._set_beyondmimic_variant("dance")
                    elif self.joy_map.g == 1.0 and self.joy_map.a == 1.0:
                        # g下拨 + A -> kick
                        self._set_beyondmimic_variant("kick")
                    elif self.joy_map.e == 1.0 and self.joy_map.a == 1.0:
                        # e右拨 + A -> 29DoF jumpover
                        self._set_beyondmimic_variant("jumpover_29dof")
                    elif self.joy_map.f == -1.0 and self.joy_map.b == 1.0:
                        # f上拨 + B -> sit_stand
                        self._set_beyondmimic_variant("sit_stand")
                    elif self.joy_map.f == 1.0 and self.joy_map.b == 1.0:
                        # f下拨 + B -> sit_chair
                        self._set_beyondmimic_variant("sit_chair")
                    elif self.joy_map.g == -1.0 and self.joy_map.b == 1.0:
                        # g上拨 + B -> squart
                        self._set_beyondmimic_variant("squart")
                    elif self.joy_map.g == 1.0 and self.joy_map.b == 1.0:
                        # g下拨 + B -> stairs
                        self._set_beyondmimic_variant("stairs")
                    elif self.joy_map.e == 1.0 and self.joy_map.b == 1.0:
                        # e右拨 + B -> jump_box
                        self._set_beyondmimic_variant("jump_box")
                    elif self.joy_map.h == 1.0 and self.joy_map.b == 1.0:
                        # h左拨 + B -> climb_stairs
                        self._set_beyondmimic_variant("climb_stairs")

            # 获取速度命令
            self.get_x_y_yaw_speed_command()
            self.get_rpyz_command()
    ...
    def get_x_y_yaw_speed_command(self):
        ...
        if self.joy_map.y1 >= 0:
            self.joy_flag.x_speed_command = (self.joy_map.y1 * self.max_x_plus_speed + 
                                            self.x_command_offset)  # 前进快一点
        else:
            self.joy_flag.x_speed_command = self.joy_map.y1 * self.max_x_minus_speed  # 后退慢一点

        # 偏航速度
        self.joy_flag.yaw_speed_command = (self.joy_map.y2 * -self.max_yaw_speed + 
    ...
    def _set_beyondmimic_variant(self, variant: str):
        """设置BeyondMimic变体并触发状态切换。"""
        self.joy_flag.beyondmimic_variant = variant
        self.joy_flag.fsm_state_command = f"gotoBEYONDMIMIC:{variant}"
