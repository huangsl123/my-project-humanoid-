...
class RobotData:
    ...
    def __init__(self, motor_num: int = 29, whole_joint_num: int = 35):
        ...
        # Floating-base state from the simulator.  Both vectors are expressed
        # in the MuJoCo world frame; policies rotate velocity into body frame.
        self.base_position_w_ = np.zeros(3, dtype=np.float32)
        self.base_linear_velocity_w_ = np.zeros(3, dtype=np.float32)
        self.base_state_received_ = False
        ...
        self.floating_base_cmd_ = np.zeros(4) # waist_roll, waist_pitch, waist_yaw, height_cmd
    ...
    def copy_from(self, other: 'RobotData'):
        ...
        self.base_position_w_[:] = other.base_position_w_[:]
        self.base_linear_velocity_w_[:] = other.base_linear_velocity_w_[:]
        self.base_state_received_ = other.base_state_received_
    ...
    def get_base_position_w(self) -> np.ndarray:
        return self.base_position_w_.copy()

    def get_base_linear_velocity_w(self) -> np.ndarray:
        return self.base_linear_velocity_w_.copy()
    ...
    def get_floating_base_cmd(self) -> np.ndarray:
        ...
        return self.floating_base_cmd_
