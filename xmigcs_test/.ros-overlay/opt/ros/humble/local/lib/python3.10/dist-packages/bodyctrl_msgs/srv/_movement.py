...
class Movement_Response(metaclass=Metaclass_Movement_Response):
    ...
    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.action_completed = kwargs.get('action_completed', bool())
    ...
    def action_completed(self, value):
        if __debug__:
            assert \
                ...
                "The 'action_completed' field must be of type 'bool'"
...
class Metaclass_Movement(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            from bodyctrl_msgs.srv import _movement
...
class Movement(metaclass=Metaclass_Movement):
    from bodyctrl_msgs.srv._movement import Movement_Request as Request
    from bodyctrl_msgs.srv._movement import Movement_Response as Response
